...
internal class ExternalApiService(HttpClient httpClient) : ApiService(httpClient)
{
    public async Task<IEnumerable<WeatherForecast>> GetAllAsync(CancellationToken cancellationToken = default)
    {
        var uri = new Uri("external", UriKind.Relative).Append("api", "weatherforecast");
    ...
    {
        var uri = new Uri("external", UriKind.Relative).Append("api", "weatherforecast", id.ToString("N"));
