    ...
    public class ClientContainer : IDisposable, IClientContainer
    {
        ...
        private T WithClient<T>(Func<ClientInfo, string, T> actionToExecute)
        {
            if (_httpContextAccessor.HttpContext.Request.RouteValues.TryGetValue("client", out var clientName) &&
                ...
                TryGetIdentifierValue(client.IdentifierHeaderName, out var identifierValue))
            {
                if (string.IsNullOrEmpty(identifierValue))
                {
                    throw new ArgumentException("Unable to determine the identifierValue");
                }

                return actionToExecute(client, identifierValue);
            }
        ...
        private bool TryGetIdentifierValue(string identifierHeaderName, out string identifierValue)
        {
            if (_httpContextAccessor.HttpContext.Request.Headers.TryGetValue(identifierHeaderName, out var values))
            {
                identifierValue = values.FirstOrDefault();
                return true;
            }

            if (_httpContextAccessor.HttpContext.Request.Headers.TryGetValue(Defaults.IdentifierHeaderName, out var defaultValues))
            {
                identifierValue = defaultValues.FirstOrDefault();
                return true;
            }
